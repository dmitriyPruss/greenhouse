...
const createNewMigrationByTemplate = (migrationFileName: string, migrationClassName: string): void => {
	const template = fs
		.readFileSync(path.join(__dirname, 'migrations/sample-migration.ts'))
		.toString()
...
const addMigrationClassToIndex = (migrationFileName: string, migrationClassName: string): void => {
	const migrationsDirIndex = fs
		.readFileSync(path.join(__dirname, 'migrations/index.ts'))
		.toString()
		.insertBeforeLastOccurrence(
			'// --imports_section_end',
			`import { ${migrationClassName} } from './${migrationFileName}';\n`
		)
		.insertBeforeLastOccurrence('// --migrations_list_end', `\tnew ${migrationClassName}(),\n`);
