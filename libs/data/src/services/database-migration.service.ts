	...
	public async migrate(migrationsList: InputMigrations<MigrationContext>): Promise<void> {
		...
		try {
			const umzug = new Umzug({
				...
				logger: {
					info: (message) => {
						DatabaseMigrationService.logger.log(message);
					},
					debug: (message) => {
						DatabaseMigrationService.logger.debug(message);
					},
					error: (message) => {
						DatabaseMigrationService.logger.error(message);
					},
					warn: (message) => {
						DatabaseMigrationService.logger.warn(message);
					},
				},
			});
